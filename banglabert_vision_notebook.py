...
#
MODEL_CFGS = [
    # ── Text Model 1: XLM-RoBERTa-Large ──────────────────────────────────
    dict(
    ...
    dict(
        ...
        type        = "text",
        epochs      = 4,        # Safely reduced to fit Kaggle 9-hr limit
        lr          = 1e-5,
        batch       = 4,
        grad_acc    = 4,
        fp16        = True,
        ...
        use_rdrop   = False,    
        rdrop_alpha = 0.0,
        use_awp     = False,    # Disabled to prevent crash risk
        awp_start   = 2,
        awp_lr      = 1e-4,
        awp_eps     = 1e-3,
        use_llrd    = True,
...
for round_idx, threshold in enumerate([PSEUDO_CFG["threshold_r1"], PSEUDO_CFG["threshold_r2"]]):
    ...
    p_args = TrainingArguments(
        ...
        gradient_accumulation_steps = best_text_cfg["grad_acc"],
...
# Leave it as is — low but not impossible
...
with zipfile.ZipFile("submission_banglabert_vision.zip", "w") as z:
