...
#
MODEL_CFGS = [
    # ── Text Model 1: MuRIL-Large ─────────────────────────────────────────
    dict(
        key         = "muril_large",
        model_name  = "google/muril-large-cased",
        type        = "text",
        epochs      = 5,
        lr          = 1e-5,     # slightly higher than XLM-R's 8e-6
        batch       = 1,        # Absolute minimum to prevent DataParallel gathering OOM
        grad_acc    = 16,       # Keeps effective batch at 16
        fp16        = True,
        grad_ckpt   = True,
        max_len     = 128,      
        use_rdrop   = False,    
        ...
        awp_start   = 2,
        awp_lr      = 1e-4,
        awp_eps     = 1e-2,     # updated to 1e-2 as per standard AWP
        use_llrd    = True,
        llrd_decay  = 0.90,
        use_ema     = False,    
        ema_decay   = 0.999,
...
def train_text_model(mcfg, train_df, test_df):
    ...
    for fold in range(SHARED["n_folds"]):
        ...
        args = TrainingArguments(
            ...
            fp16                        = mcfg["fp16"],
            optim                       = "adafactor",  # <--- CRITICAL VRAM SAVER
        ...
        # Collect OOF predictions for temperature calibration
        val_preds = trainer.predict(tok_val).predictions
        val_indices = train_df[train_df["fold"] == fold].index
        oof_logits[val_indices] = val_preds
        oof_labels[val_indices] = val_df["label_id"].values
    ...
    return np.array(fold_preds), mean_f1, oof_logits, oof_labels
...
all_oof    = {}   # key -> {"logits": ..., "labels": ...} for temperature calibration

for mcfg in MODEL_CFGS:
    seed_everything(SHARED["seed"])

    if mcfg["type"] == "text":
        preds, cv_f1, oof_logits, oof_labels = train_text_model(mcfg, train, test)
        all_oof[mcfg["key"]] = {"logits": oof_logits, "labels": oof_labels}
    else:
...
# (Temperature calibration removed to prevent flattening logits before pseudo-labeling)
...
# ═══════════════════════════════════════════════════════════════════════════════
PSEUDO_CFG = dict(
    threshold_r1 = 0.78,   # lowered to guarantee trigger
    threshold_r2 = 0.75,   # lowered to guarantee trigger
    epochs       = 3,
...
# ── Suppress impossible/near-impossible combinations ─────────────────────
...
with zipfile.ZipFile("submission_ultimate_v5.zip", "w") as z:
